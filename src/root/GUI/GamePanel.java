...
import root.GameMode.Stage.Stage;
...
public class GamePanel extends JPanel implements Runnable, MouseInputListener, CellsManager, ResourcesPath {
    ...
    private void resetToLevel(int levelNumber) {
        ...
        visualMode = Stage.getVisualMode(this.levelNumber);
        level = Stage.getStage(this.levelNumber, this);
        message.removeAll();
        message.setText("");
        initMessage();
        ...
        Sun.Count = 0;
        if (levelNumber == 0) {
            Sun.updateCount(2000);
        } else Sun.updateCount(3000);
    ...
    private void cycle() {
        ...
        level.movableObjects.forEach(MovableObjects::actions);
        ...
        if (!level.end()) {
            ...
            if (level.zombieKilled >= level.MAX_ZOMBIES) {
                ...
                VisualMode new_visualMode = Stage.getVisualMode(levelNumber + 1);
                if (new_visualMode != null) {
                    visualMode = new_visualMode;
                    Stage new_level = Stage.getStage(levelNumber++, this);
                    if (new_level != null) {
    ...
    public void paintComponent(Graphics g) {
        ...
        level.movableObjects.forEach((stuff) -> {
            g.drawImage(stuff.getImage(), stuff.getX(), stuff.getY(), null);
//      if(stuff instanceof HeadlessSun){
//
//      }
            if (testMode) {
                g.drawRect(stuff.getBounds().x, stuff.getBounds().y, stuff.getBounds().width, stuff.getBounds().height);
    ...
    public void mouseClicked(MouseEvent mouseEvent) {
        for (MovableObjects movableObjects : level.movableObjects) {
                            ...
                            try {
                                ...
                                if (tmp_plant.getImage().equals(source.dragTarget)) {
                                    if (Sun.Count >= tmp_plant.price) {
                                        if (tmp_plant.getImage().equals(visualMode.GatlingPeaImage)) {
                                            cellMaps.forEach((point, plant) -> {
                                                if (plant.getImage().equals(visualMode.RepeaterImage)) {
                                                    try {
                                                        cellMaps.remove(cell.getLocation());
